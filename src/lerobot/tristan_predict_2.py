...
def predict(model):
    ...
    for ep in range(5):  # Run 5 episodes
        ...
        while not done:
            ...
            with torch.no_grad():
                pred_action = model.select_action(batch).cpu().numpy()
            pred_action = pred_action[0]

            # Denormalize
            pred_action = 0.5 * (pred_action + 1.0) * (ACTION_MAX - ACTION_MIN) + ACTION_MIN

            true_action = env.get_action(frame_index=frame_index)
            frame_index += 1
